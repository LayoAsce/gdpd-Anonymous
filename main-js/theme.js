...
(function($) {
    ...
    navbarFixed();
    $('.main_menu .navbar-nav li a[href^="#"]:not([href="#"]').on('click', function(event) { var $anchor = $(this);
        $('html, body').stop().animate({ scrollTop: $($anchor.attr('href')).offset().top - 70 }, 1500);
        event.preventDefault(); });
    $(window).on('load', function() { $('body').scrollspy({ target: '#mainNav', offset: 70 }); });
    ...
    function testimonials_slider() { if ($('.testi_slider').length) { $('.testi_slider').owlCarousel({ loop: true, margin: 30, items: 2, nav: false, autoplay: true, smartSpeed: 1500, dots: false, responsiveClass: true, responsive: { 0: { items: 1, }, 768: { items: 2, }, } }) } }
    testimonials_slider();

    function screenshot_slider() { if ($('.screenshot_inner').length) { $('.screenshot_inner').owlCarousel({ loop: true, margin: 30, items: 4, nav: false, autoplay: true, smartSpeed: 1500, dots: false, responsiveClass: true, responsive: { 0: { items: 2, }, 576: { items: 4, }, } }) } }
    screenshot_slider();
